classdef ExpCatheterDeconvolution3 < mlaif.AbstractAifProblem & mlarbelaez.AbstractCatheterAnalysis 
	...
	methods 
        function this  = estimateParameters(this)
            ...
            map('alpha2') = struct('fixed', 1, 'min', eps0, 'mean',  0.45, 'max',  5);
            map('beta')   = struct('fixed', 0, 'min', eps0, 'mean',  0.23, 'max', 10);
            map('beta2')  = struct('fixed', 1, 'min', eps0, 'mean',  0.23, 'max', 10);
            map('c1')     = struct('fixed', 0, 'min', -100, 'mean',  0,    'max', 100);
            map('c2')     = struct('fixed', 1, 'min', -100, 'mean',  0,    'max', 100);
            map('c3')     = struct('fixed', 1, 'min', -100, 'mean',  0,    'max', 100);
            map('c4')     = struct('fixed', 1, 'min', -100, 'mean',  0,    'max', 100);
            map('delta')  = struct('fixed', 0, 'min', eps0, 'mean',  0.03, 'max',   1);
            map('eps')    = struct('fixed', 1, 'min', eps0, 'mean',  0,    'max',  0.33);
            ...
            map('tauRe')  = struct('fixed', 1, 'min', eps0, 'mean',  0,    'max', 20);
    ...
    methods (Access = 'protected')        
        function dccrv = estimateDccrvFast(this, alpha_, alpha2_, beta_, beta2_, c1, c2, c3, c4, delta_, eps_, fracSS, gamma_, ncnt, t0, tauRe)
            dccrv = conv(this.estimateDcvFast(alpha_, alpha2_, beta_, beta2_, c1, c2, c3, c4, delta_, eps_, fracSS, gamma_, ncnt, t0, tauRe), this.response);
            dccrv = this.ensureDccrvLength(dccrv);
        end
        function dcv   = estimateDcvFast(  this, alpha_, alpha2_, beta_, beta2_, c1, c2, c3, c4, delta_, eps_, fracSS, gamma_, ncnt, t0, tauRe)
            bp1 = this.gammaVariateFast(alpha_, beta_, t0);
            bp2 = this.gammaVariateFast(alpha2_, beta2_, t0 + tauRe);
            cp1 = (1 - fracSS) * (1 - eps_)  * bp1;
            cp2 = (1 - fracSS) * eps_ * bp2;
            css =      fracSS  * max(cp1) * this.bolusSteadyStateFast(gamma_, t0);
            dcv = ncnt * (cp1 + cp2 + css) .* this.expPolyFast(c1, c2, c3, c4, delta_, t0);
