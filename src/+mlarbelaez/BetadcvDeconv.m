classdef BetadcvDeconv
	...
	methods 		
        ...
        function [f, n] = expandData(this, f, n)
 		...
 		function this = BetadcvDeconv(varargin) 
 			%% BETADCVDECONV 
 			%  Usage:  this = BetadcvDeconv([dccrv_filename]) 
            
            ip = inputParser;
            addOptional(ip, 'dccrvFn', 'AMAtest4.crv', @(x) lexist(x, 'file'));
            parse(ip, varargin{:});
            
            this.dccrv_  = mlpet.DecayCorrectedCRV.load(ip.Results.dccrvFn);
